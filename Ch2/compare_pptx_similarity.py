...
if len(texts) > 1 and any(len(t) > 0 for t in texts):
    try:
        ...
        with open('similarity_report.txt', 'w', encoding='utf-8') as f:
            ...
            # Deduction points for each file based on average similarity
            f.write("DEDUCTION POINTS (1-5) BASED ON AVERAGE SIMILARITY:\n")
            f.write("-" * 48 + "\n")
            for file in pptx_files:
                # Average similarity with other files
                sim_sum = 0
                count = 0
                for other in pptx_files:
                    if other != file:
                        sim_sum += similarity_df.loc[file, other]
                        count += 1
                ...
                if avg_sim < 30:
                    ...
                    points = 5
                f.write(f"{file}: Average similarity {avg_sim:.2f}% -> Deduction: {points} points\n")
            f.write("\n")
